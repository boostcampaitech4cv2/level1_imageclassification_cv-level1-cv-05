...
def parse_args():
    ...
    parser.add_argument("--resize", nargs="+", type=list,
                        default=[384, 384], help='resize size for image when training')
    ...
    parser.add_argument('--name', default='Swin_Large_Weighted_Profile_bbox_58_KFold',
                        help='model save at {SM_MODEL_DIR}/{name}')
    parser.add_argument('--weightedsampler', type=str, default='yes',
                        help='weighted sampler (default: no  (no, yes))')
    ...
    parser.add_argument('--rembg_dir', type=str, default=os.environ.get(
        'SM_CHANNEL_TRAIN', '/opt/ml/input/data/train/images_rembg'))
